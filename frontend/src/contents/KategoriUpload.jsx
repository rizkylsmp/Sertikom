...
const KategoriUpload = () => {
    ...
    const fetchNextId = async () => {
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_API_BASE_URL}/nextkategori`
        );
        setFormData((prevFormData) => ({
          ...prevFormData,
          id: response.data.nextId,
        }));
      } catch (error) {
        console.error("Error fetching next ID:", error);
        window.alert("Error fetching next ID");
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setIsLoading(true);
      await axios.post(`${process.env.REACT_APP_API_BASE_URL}/kategori`, {
        nama: formData.nama,
